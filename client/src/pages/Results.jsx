  ...
  if (!results) {
    return (
      <div className="min-h-screen bg-[#0a0a1a] flex flex-col items-center justify-center text-white overflow-hidden">
        ...
        <p className="text-3xl text-slate-500 font-light uppercase tracking-widest">Waiting for final results...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#0a0a1a] text-white p-8 font-sans overflow-x-hidden">
      ...
      <div className="max-w-5xl mx-auto space-y-5">
        <AnimatePresence>
          {results.map((song, index) => (
            ...
            >
              {index === 0 && (
                  <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/20 to-transparent pointer-events-none" />
              )}
              <div className={`flex-shrink-0 w-20 text-5xl font-black mr-6 text-center ${index === 0 ? 'text-yellow-400' : index === 1 ? 'text-slate-300' : index === 2 ? 'text-amber-600' : 'text-slate-600'}`}>
                {index + 1}
              </div>
              <div className="flex-grow z-10">
                <div className="text-4xl font-bold mb-1">{song.name}</div>
                <div className="text-2xl text-slate-400 font-medium">{song.artist}</div>
              </div>
              <div className="flex-shrink-0 text-right z-10">
                ...
                <div className="text-sm text-slate-500 uppercase tracking-widest font-bold mt-1">Points</div>
              </div>
            </motion.div>
          ))}
        </AnimatePresence>
